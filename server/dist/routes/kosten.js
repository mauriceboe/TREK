...
function getTripMembers(tripId) {
  ...
  `).all(tripId);
    ...
    const seen = new Set();
    return all.filter(m => { if (seen.has(m.id))
...
function loadFullExpense(id) {
    const expense = database_1.db.prepare(`
  ...
  `).get(id);
    if (!expense)
        return undefined;
    expense.shares = loadExpenseShares(expense.id).map((s) => ({ ...s, avatar_url: avatarUrl(s) }));
...
router.get('/', auth_1.authenticate, (req, res) => {
    ...
    if (!(0, database_1.canAccessTrip)(tripId, authReq.user.id))
        return res.status(404).json({ error: 'Trip not found' });
    const expenses = database_1.db.prepare(`
    SELECT ke.*, COALESCE(ke.paid_by_name, u.username) as paid_by_username, u.avatar as paid_by_avatar
    FROM kosten_expenses ke LEFT JOIN users u ON ke.paid_by = u.id
    WHERE ke.trip_id = ?
  ...
  `).all(tripId);
    ...
    if (expenseIds.length > 0) {
        const allShares = database_1.db.prepare(`
    ...
    `).all(...expenseIds);
        ...
        for (const s of allShares) {
            if (!sharesByExpense[s.expense_id])
                sharesByExpense[s.expense_id] = [];
        ...
        expenses.forEach((e) => { e.shares = []; e.paid_by_avatar_url = null; });
    }
    res.json({ expenses });
});
// ── POST / — create expense ──────────────────────────────────────────────────
router.post('/', auth_1.authenticate, (req, res) => {
  ...
  `).run(tripId, title, Number(amount), currency || tripCurrency, Number(exchange_rate), paid_by ? Number(paid_by) : null, paid_by_name || null, category, expense_date || null, note || null, split_type);
    ...
    const insertShare = database_1.db.prepare('INSERT OR IGNORE INTO kosten_shares (expense_id, user_id, share_value) VALUES (?, ?, ?)');
    for (const uid of participants) {
        insertShare.run(expenseId, uid, null);
    }
    const expense = loadFullExpense(expenseId);
    (0, websocket_1.broadcast)(Number(tripId), 'kosten:created', { expense }, req.headers['x-socket-id']);
...
router.put('/:id', auth_1.authenticate, (req, res) => {
    ...
    if (!(0, database_1.canAccessTrip)(tripId, authReq.user.id))
        return res.status(404).json({ error: 'Trip not found' });
    const existing = database_1.db.prepare('SELECT id FROM kosten_expenses WHERE id = ? AND trip_id = ?').get(id, tripId);
    if (!existing)
    ...
    const allowed = ['title', 'amount', 'currency', 'exchange_rate', 'paid_by', 'paid_by_name', 'category', 'expense_date', 'note', 'split_type'];
    const fields = [];
    const values = [];
    ...
    database_1.db.prepare(`UPDATE kosten_expenses SET ${fields.join(', ')} WHERE id = ? AND trip_id = ?`).run(...values, id, tripId);
    const expense = loadFullExpense(Number(id));
    (0, websocket_1.broadcast)(Number(tripId), 'kosten:updated', { expense }, req.headers['x-socket-id']);
    res.json({ expense });
});
// ── DELETE /:id ──────────────────────────────────────────────────────────────
router.delete('/:id', auth_1.authenticate, (req, res) => {
    ...
    if (!existing)
        return res.status(404).json({ error: 'Expense not found' });
    database_1.db.prepare('DELETE FROM kosten_expenses WHERE id = ? AND trip_id = ?').run(id, tripId);
    (0, websocket_1.broadcast)(Number(tripId), 'kosten:deleted', { id: Number(id) }, req.headers['x-socket-id']);
    res.json({ success: true });
});
// ── PUT /:id/shares — update participants + share values ─────────────────────
router.put('/:id/shares', auth_1.authenticate, (req, res) => {
    const authReq = req;
    const { tripId, id } = req.params;
    if (!(0, database_1.canAccessTrip)(tripId, authReq.user.id))
        return res.status(404).json({ error: 'Trip not found' });
    ...
    database_1.db.prepare('DELETE FROM kosten_shares WHERE expense_id = ?').run(id);
    const insertShare = database_1.db.prepare('INSERT INTO kosten_shares (expense_id, user_id, share_value) VALUES (?, ?, ?)');
...
router.get('/balances', auth_1.authenticate, (req, res) => {
    const authReq = req;
    const { tripId } = req.params;
    if (!(0, database_1.canAccessTrip)(tripId, authReq.user.id))
        return res.status(404).json({ error: 'Trip not found' });
  ...
  `).all(tripId);
    const allShareRows = database_1.db.prepare(`
  ...
  `).all(tripId);
    ...
    const settlements = database_1.db.prepare(`
  ...
  `).all(tripId);
    ...
    if (allUserIds.length > 0) {
        const users = database_1.db.prepare(`SELECT id, username, avatar FROM users WHERE id IN (${allUserIds.map(() => '?').join(',')})`).all(...allUserIds);
    ...
    while (ci < cred.length && di < debt.length) {
        ...
        c.balance -= transfer;
        d.balance += transfer;
        if (Math.abs(c.balance) < 0.005)
            ci++;
        if (Math.abs(d.balance) < 0.005)
            di++;
    }
    res.json({ balances: balanceList, debts });
});
// ── GET /settlements ─────────────────────────────────────────────────────────
router.get('/settlements', auth_1.authenticate, (req, res) => {
    const authReq = req;
    const { tripId } = req.params;
    if (!(0, database_1.canAccessTrip)(tripId, authReq.user.id))
        return res.status(404).json({ error: 'Trip not found' });
...
// ── POST /settlements ────────────────────────────────────────────────────────
router.post('/settlements', auth_1.authenticate, (req, res) => {
    const authReq = req;
    const { tripId } = req.params;
    if (!(0, database_1.canAccessTrip)(tripId, authReq.user.id))
        return res.status(404).json({ error: 'Trip not found' });
  ...
  `).run(tripId, from_user_id ? Number(from_user_id) : null, from_name || null, to_user_id ? Number(to_user_id) : null, to_name || null, Number(amount), currency || tripCurrency, Number(exchange_rate), note || null);
    const settlement = database_1.db.prepare(`
    SELECT ks.*,
      COALESCE(ks.from_name, uf.username) as from_username, uf.avatar as from_avatar,
      COALESCE(ks.to_name, ut.username) as to_username, ut.avatar as to_avatar
    FROM kosten_settlements ks
  ...
  `).get(result.lastInsertRowid);
    ...
    (0, websocket_1.broadcast)(Number(tripId), 'kosten:settlement_created', { settlement: s }, req.headers['x-socket-id']);
    res.status(201).json({ settlement: s });
});
// ── DELETE /settlements/:id ──────────────────────────────────────────────────
router.delete('/settlements/:id', auth_1.authenticate, (req, res) => {
    const authReq = req;
    const { tripId, id } = req.params;
    if (!(0, database_1.canAccessTrip)(tripId, authReq.user.id))
        return res.status(404).json({ error: 'Trip not found' });
    const existing = database_1.db.prepare('SELECT id FROM kosten_settlements WHERE id = ? AND trip_id = ?').get(id, tripId);
    if (!existing)
        return res.status(404).json({ error: 'Settlement not found' });
    database_1.db.prepare('DELETE FROM kosten_settlements WHERE id = ? AND trip_id = ?').run(id, tripId);
    (0, websocket_1.broadcast)(Number(tripId), 'kosten:settlement_deleted', { id: Number(id) }, req.headers['x-socket-id']);
    res.json({ success: true });
});
// ── GET /exchange-rate — proxy Frankfurter API ────────────────────────────────
router.get('/exchange-rate', auth_1.authenticate, async (req, res) => {
    ...
    if (fromUpper === toUpper)
        return res.json({ rate: 1, from: fromUpper, to: toUpper });
    try {
        const resp = await (0, node_fetch_1.default)(`https://api.frankfurter.app/latest?from=${encodeURIComponent(fromUpper)}&to=${encodeURIComponent(toUpper)}`);
        if (!resp.ok)
...
//# sourceMappingURL=kosten.js.map