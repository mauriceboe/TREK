...
async function fetchOverpassDetails(osmType, osmId) {
    ...
    try {
        const res = await (0, node_fetch_1.default)('https://overpass-api.de/api/interpreter', {
            method: 'POST',
...
async function fetchWikimediaPhoto(lat, lng, name) {
    ...
    if (name) {
        try {
            ...
            const res = await (0, node_fetch_1.default)(`https://en.wikipedia.org/w/api.php?${searchParams}`, { headers: { 'User-Agent': UA } });
            if (res.ok) {
                const data = await res.json();
    ...
    try {
        const res = await (0, node_fetch_1.default)(`https://commons.wikimedia.org/w/api.php?${params}`, { headers: { 'User-Agent': UA } });
        if (!res.ok)
            return null;
        const data = await res.json();
        const pages = data.query?.pages;
...
setInterval(() => {
    ...
    for (const [key, entry] of photoCache) {
        if (now - entry.fetchedAt > PHOTO_TTL)
...
async function searchNominatim(query, lang) {
    ...
    const response = await (0, node_fetch_1.default)(`https://nominatim.openstreetmap.org/search?${params}`, {
...
router.post('/search', auth_1.authenticate, async (req, res) => {
    ...
    try {
        const response = await (0, node_fetch_1.default)('https://places.googleapis.com/v1/places:searchText', {
            method: 'POST',
            headers: {
...
router.get('/details/:placeId', auth_1.authenticate, async (req, res) => {
    const authReq = req;
    ...
    if (placeId.includes(':')) {
        ...
        try {
            ...
            if (!element?.tags)
                return res.json({ place: buildOsmDetails({}, osmType, osmId) });
    ...
    try {
        ...
        const response = await (0, node_fetch_1.default)(`https://places.googleapis.com/v1/places/${placeId}?languageCode=${lang}`, {
            method: 'GET',
            headers: {
                'X-Goog-Api-Key': apiKey,
...
router.get('/place-photo/:placeId', auth_1.authenticate, async (req, res) => {
    const authReq = req;
    const { placeId } = req.params;
    ...
    if (!apiKey || isCoordLookup) {
        if (!isNaN(lat) && !isNaN(lng)) {
            try {
                const wiki = await fetchWikimediaPhoto(lat, lng, req.query.name);
                if (wiki) {
    ...
    // Google Photos
    try {
        const detailsRes = await (0, node_fetch_1.default)(`https://places.googleapis.com/v1/places/${placeId}`, {
            headers: {
                'X-Goog-Api-Key': apiKey,
        ...
        const mediaRes = await (0, node_fetch_1.default)(`https://places.googleapis.com/v1/${photoName}/media?maxHeightPx=600&key=${apiKey}&skipHttpRedirect=true`);
        ...
        try {
            database_1.db.prepare('UPDATE places SET image_url = ?, updated_at = CURRENT_TIMESTAMP WHERE google_place_id = ? AND (image_url IS NULL OR image_url = ?)').run(photoUrl, placeId, '');
        }
        catch (dbErr) {
...
router.get('/reverse', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        const response = await (0, node_fetch_1.default)(`https://nominatim.openstreetmap.org/reverse?${params}`, {
            headers: { 'User-Agent': UA },
        });
        if (!response.ok)
            return res.json({ name: null, address: null });
        const data = await response.json();
