...
router.get('/trips/:tripId/days/:dayId/assignments', auth_1.authenticate, tripAccess_1.requireTripAccess, (req, res) => {
...
router.post('/trips/:tripId/days/:dayId/assignments', auth_1.authenticate, tripAccess_1.requireTripAccess, (req, res) => {
    const { tripId, dayId } = req.params;
    ...
    const result = database_1.db.prepare('INSERT INTO day_assignments (day_id, place_id, order_index, notes) VALUES (?, ?, ?, ?)').run(dayId, place_id, orderIndex, notes || null);
    const assignment = getAssignmentWithPlace(result.lastInsertRowid);
    res.status(201).json({ assignment });
    (0, websocket_1.broadcast)(tripId, 'assignment:created', { assignment }, req.headers['x-socket-id']);
});
router.delete('/trips/:tripId/days/:dayId/assignments/:id', auth_1.authenticate, tripAccess_1.requireTripAccess, (req, res) => {
    const { tripId, dayId, id } = req.params;
    const assignment = database_1.db.prepare('SELECT da.id FROM day_assignments da JOIN days d ON da.day_id = d.id WHERE da.id = ? AND da.day_id = ? AND d.trip_id = ?').get(id, dayId, tripId);
    if (!assignment)
        return res.status(404).json({ error: 'Assignment not found' });
    database_1.db.prepare('DELETE FROM day_assignments WHERE id = ?').run(id);
    res.json({ success: true });
    (0, websocket_1.broadcast)(tripId, 'assignment:deleted', { assignmentId: Number(id), dayId: Number(dayId) }, req.headers['x-socket-id']);
});
router.put('/trips/:tripId/days/:dayId/assignments/reorder', auth_1.authenticate, tripAccess_1.requireTripAccess, (req, res) => {
    const { tripId, dayId } = req.params;
    ...
    res.json({ success: true });
    (0, websocket_1.broadcast)(tripId, 'assignment:reordered', { dayId: Number(dayId), orderedIds }, req.headers['x-socket-id']);
});
router.put('/trips/:tripId/assignments/:id/move', auth_1.authenticate, tripAccess_1.requireTripAccess, (req, res) => {
  ...
  `).get(id, tripId);
    if (!assignment)
        return res.status(404).json({ error: 'Assignment not found' });
    const newDay = database_1.db.prepare('SELECT id FROM days WHERE id = ? AND trip_id = ?').get(new_day_id, tripId);
    if (!newDay)
        return res.status(404).json({ error: 'Target day not found' });
    const oldDayId = assignment.day_id;
    database_1.db.prepare('UPDATE day_assignments SET day_id = ?, order_index = ? WHERE id = ?').run(new_day_id, order_index || 0, id);
    const updated = getAssignmentWithPlace(Number(id));
    res.json({ assignment: updated });
    (0, websocket_1.broadcast)(tripId, 'assignment:moved', { assignment: updated, oldDayId: Number(oldDayId), newDayId: Number(new_day_id) }, req.headers['x-socket-id']);
});
router.get('/trips/:tripId/assignments/:id/participants', auth_1.authenticate, tripAccess_1.requireTripAccess, (req, res) => {
    const { tripId, id } = req.params;
    const participants = database_1.db.prepare(`
    SELECT ap.user_id, u.username, u.avatar
    FROM assignment_participants ap
...
router.put('/trips/:tripId/assignments/:id/time', auth_1.authenticate, tripAccess_1.requireTripAccess, (req, res) => {
    const { tripId, id } = req.params;
    const assignment = database_1.db.prepare(`
    SELECT da.* FROM day_assignments da
    JOIN days d ON da.day_id = d.id
  ...
  `).get(id, tripId);
    ...
    const updated = getAssignmentWithPlace(Number(id));
    res.json({ assignment: updated });
    (0, websocket_1.broadcast)(Number(tripId), 'assignment:updated', { assignment: updated }, req.headers['x-socket-id']);
});
router.put('/trips/:tripId/assignments/:id/participants', auth_1.authenticate, tripAccess_1.requireTripAccess, (req, res) => {
    const { tripId, id } = req.params;
    const { user_ids } = req.body;
    if (!Array.isArray(user_ids))
        return res.status(400).json({ error: 'user_ids must be an array' });
    database_1.db.prepare('DELETE FROM assignment_participants WHERE assignment_id = ?').run(id);
  ...
  `).all(id);
    res.json({ participants });
    (0, websocket_1.broadcast)(Number(tripId), 'assignment:participants', { assignmentId: Number(id), participants }, req.headers['x-socket-id']);
});
