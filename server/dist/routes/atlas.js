...
function getCountryFromAddress(address) {
    ...
    if (NAME_TO_CODE[last])
        return NAME_TO_CODE[last];
...
router.get('/stats', (req, res) => {
  ...
  `).all(userId, userId, userId);
    ...
    const places = database_1.db.prepare(`SELECT * FROM places WHERE trip_id IN (${placeholders})`).all(...tripIds);
...
router.get('/country/:code', (req, res) => {
  ...
  `).all(userId, userId, userId);
    ...
    for (const place of places) {
        ...
        if (!pCode && place.lat && place.lng)
            pCode = getCountryFromCoords(place.lat, place.lng);
        if (pCode === code) {
    ...
    const isManuallyMarked = !!(database_1.db.prepare('SELECT 1 FROM visited_countries WHERE user_id = ? AND country_code = ?').get(userId, code));
    res.json({ places: matchingPlaces, trips: matchingTrips, manually_marked: isManuallyMarked });
});
// Mark/unmark country as visited
router.post('/country/:code/mark', (req, res) => {
    const authReq = req;
    database_1.db.prepare('INSERT OR IGNORE INTO visited_countries (user_id, country_code) VALUES (?, ?)').run(authReq.user.id, req.params.code.toUpperCase());
    res.json({ success: true });
});
router.delete('/country/:code/mark', (req, res) => {
    const authReq = req;
    database_1.db.prepare('DELETE FROM visited_countries WHERE user_id = ? AND country_code = ?').run(authReq.user.id, req.params.code.toUpperCase());
    res.json({ success: true });
});
// ── Bucket List ─────────────────────────────────────────────────────────────
router.get('/bucket-list', (req, res) => {
    const authReq = req;
    const items = database_1.db.prepare('SELECT * FROM bucket_list WHERE user_id = ? ORDER BY created_at DESC').all(authReq.user.id);
    res.json({ items });
});
router.post('/bucket-list', (req, res) => {
    ...
    const result = database_1.db.prepare('INSERT INTO bucket_list (user_id, name, lat, lng, country_code, notes) VALUES (?, ?, ?, ?, ?, ?)').run(authReq.user.id, name.trim(), lat ?? null, lng ?? null, country_code ?? null, notes ?? null);
...
router.put('/bucket-list/:id', (req, res) => {
    ...
    const item = database_1.db.prepare('SELECT * FROM bucket_list WHERE id = ? AND user_id = ?').get(req.params.id, authReq.user.id);
    if (!item)
        return res.status(404).json({ error: 'Item not found' });
    database_1.db.prepare('UPDATE bucket_list SET name = COALESCE(?, name), notes = COALESCE(?, notes) WHERE id = ?').run(name?.trim() || null, notes ?? null, req.params.id);
    res.json({ item: database_1.db.prepare('SELECT * FROM bucket_list WHERE id = ?').get(req.params.id) });
});
router.delete('/bucket-list/:id', (req, res) => {
    ...
    if (!item)
        return res.status(404).json({ error: 'Item not found' });
    database_1.db.prepare('DELETE FROM bucket_list WHERE id = ?').run(req.params.id);
    res.json({ success: true });
});
