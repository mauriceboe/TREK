...
// ── Inline Edit Cell ─────────────────────────────────────────────────────────
function InlineEditCell({ value, onSave, type = 'text', style = {} as Record<string, any>, placeholder = '', decimals = 2, locale, editTooltip, readOnly = false }) {
...
export default function BudgetPanel({ tripId, tripMembers = [] }: BudgetPanelProps) {
  ...
  const th = { padding: '6px 8px', textAlign: 'center' as const, fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase' as const, letterSpacing: '0.05em', borderBottom: '2px solid var(--border-primary)', whiteSpace: 'nowrap' as const, background: 'var(--bg-secondary)' }
