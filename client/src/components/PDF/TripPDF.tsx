...
async function fetchPlacePhotos(assignments) {
  ...
  await Promise.allSettled(
    toFetch.map(async (place) => {
      ...
      } catch { /* photo fetch failure is non-critical for PDF */ }
    })
  )
  return photoMap
}
...
function pdfFormatFuelCost(cost: number, currency: string): string {
  ...
  } catch { /* expected for unsupported currency codes */ return `${cost.toFixed(2)} ${currency || 'USD'}` }
}
        ...
        if (directions.length > 0) {
          if ((l.distance_meters || 0) > 800000) {
            ...
            directionsHtml = directions.map(d => {
              const sym = dirSymbol(d.maneuver, d.instruction)
