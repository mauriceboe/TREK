  ...
  useEffect(() => {
    if (days.length > prevDayCount.current) {
      ...
      setExpandedDays(prev => {
        ...
        try { sessionStorage.setItem(`day-expanded-${tripId}`, JSON.stringify([...n])) } catch { /* sessionStorage may be unavailable */ }
        return n
      })
    }
          ...
          return (
            <div key={day.id} style={{ borderBottom: '1px solid var(--border-faint)' }}>
              ...
              {roadtripEnabled && isExpanded && (() => {
                ...
                const dayLegs = roadtripStore.getLegsForDay(String(tripId), dayNum).filter(l => l.is_road_trip)
                if (dayLegs.some(l => !l.duration_seconds)) return null
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              {roadtripEnabled && placeIdx > 0 && (() => {
                                ...
                                const foundStops = (() => {
                                  ...
                                  } catch { /* expected when route_metadata is malformed */ return [] }
                                })()
