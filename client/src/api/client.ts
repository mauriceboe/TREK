...
export const kostenApi = {
  list: (tripId: number | string) => apiClient.get(`/trips/${tripId}/kosten`).then(r => r.data),
  ...
  deleteSettlement: (tripId: number | string, id: number) => apiClient.delete(`/trips/${tripId}/kosten/settlements/${id}`).then(r => r.data),
  getExchangeRate: (tripId: number | string, from: string, to: string) => apiClient.get(`/trips/${tripId}/kosten/exchange-rate`, { params: { from, to } }).then(r => r.data),
}
